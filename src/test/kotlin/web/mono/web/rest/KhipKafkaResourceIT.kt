...
import web.mono.config.KafkaProperties

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class KhipKafkaResourceIT {
    ...
    fun setup() {
        ...
        val kafkaResource = KhipKafkaResource(kafkaProperties)
    ...
    fun producesMessages() {
        restMockMvc.perform(post("/api/khip-kafka/publish/topic-produce?message=value-produce"))
    ...
    fun consumesMessages() {
        ...
        val mvcResult = restMockMvc.perform(get("/api/khip-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
